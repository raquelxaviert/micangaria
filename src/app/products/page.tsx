...
function ProductCard({ product }: { product: Product }) {
  return (
    <Card id={product.id} className="flex flex-col h-full shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-lg overflow-hidden group">
      <CardHeader className="p-0 relative">
        <div className="aspect-video w-full overflow-hidden">
         ...
         {product.isNewArrival && (
          <div className="absolute top-2 left-2 bg-accent text-accent-foreground px-2 py-1 text-xs font-semibold rounded">NEW</div>
        )}
      </CardHeader>
      ...
      <CardFooter className="p-4 pt-0">
         {/* Placeholder for future "Add to Cart" or specific product page link */}
...
function FilterControls({ filters, setFilters }: { filters: Filters, setFilters: (filters: Filters) => void}) {
  ...
  return (
    <div className="space-y-6 p-4">
      <div>
        <h3 className="font-semibold mb-2 text-lg font-headline">Search</h3>
        <Input
          type="text"
      ...
      <div>
        <h3 className="font-semibold mb-2 text-lg font-headline">Type</h3>
        <Select
          value={filters.type || ''}
      ...
      <div>
        <h3 className="font-semibold mb-2 text-lg font-headline">Style</h3>
        <Select
          value={filters.style || ''}
      ...
      <div>
        <h3 className="font-semibold mb-2 text-lg font-headline">Color</h3>
      ...
      <div>
        <h3 className="font-semibold mb-2 text-lg font-headline">Special</h3>
        <div className="flex items-center space-x-2 mb-2">
          ...
          />
          <Label htmlFor="showNew">New Arrivals</Label>
        </div>
        <div className="flex items-center space-x-2">
          ...
          />
          <Label htmlFor="showPromotions">Promotions</Label>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8">
      ...
      <aside className="hidden md:block md:w-1/4 lg:w-1/5 sticky top-20 h-screen-minus-header-footer overflow-y-auto bg-card p-4 rounded-lg shadow-md">
        <h2 className="text-2xl font-headline text-primary mb-6 flex items-center"><Filter className="mr-2" />Filters</h2>
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button variant="outline" className="w-full text-primary border-primary">
              <ListFilter className="mr-2 h-4 w-4" /> Show Filters
            </Button>
          </SheetTrigger>
          <SheetContent side="left" className="w-[300px] sm:w-[400px] overflow-y-auto bg-background">
            <SheetHeader>
              <SheetTitle className="text-2xl font-headline text-primary flex items-center mt-4"><Filter className="mr-2" />Filters</SheetTitle>
            </SheetHeader>
            <FilterControls filters={filters} setFilters={setFilters} />
          </SheetContent>
        </Sheet>
      ...
      <main className="flex-1">
        <h1 className="text-4xl font-headline text-primary mb-8">Our Collection</h1>
        {filteredProducts.length > 0 ? (
          ...
          <p className="text-center text-muted-foreground text-lg col-span-full">No products match your current filters. Try adjusting your selection!</p>
        )}
      </main>
    </div>
  );
